...
window.DLDOSSIER = (function () {
  ...
  function dossier() {
    ...
    detail(c.id).then(function (r) {
      ...
      el.innerHTML = '<div class="h">📁 Dossier · ' + esc(sym) + '<span class="sp"></span>' + C.prov("CoinGecko", r.at, r.stale) + "</div>" +
        (st ? '<div style="font:500 13px var(--cmc-sans,sans-serif);line-height:1.5;margin-bottom:8px">' + esc(st) + "</div>" : "") +
        (links.length ? '<div class="dl-chips" style="margin:0 0 8px">' + links.map(function (l) { return '<a class="dl-chip" href="' + esc(l[1]) + '" target="_blank" rel="noopener noreferrer" style="text-decoration:none">' + esc(l[0]) + " ↗</a>"; }).join("") + "</div>" : "") +
        ...
        (chains.length ? '<div class="h" style="margin:10px 0 4px">Contracts <span style="font:500 11px var(--cmc-sans,sans-serif);color:var(--muted)">· one tap scans it on GoPlus</span></div><div class="dl-chips" style="margin:0">' + chains.map(function (k) { return '<button class="dl-chip" data-scan="' + esc(k) + '" data-addr="' + esc(plat[k]) + '" title="' + esc(plat[k]) + '">🛡 ' + esc(k) + "</button>"; }).join("") + "</div>" : "") +
  ...
  function wire() {
    ...
    el.innerHTML = '<div class="h">📰 Coin Wire · ' + esc(sym) + ' <span style="font:500 11px var(--cmc-sans,sans-serif);color:var(--muted)">· the News desk filtered to this coin</span><span class="sp"></span>' + (nc.length ? '<span class="dl-prov recent">' + nc.length + " headlines loaded</span>" : '<button class="dl-b" id="dlWireLoad">Load the desk</button>') + "</div>" +
      (hits.length ? hits.map(function (a) { var s = sent(a.title); return '<div class="kv" style="align-items:flex-start"><span><a href="' + esc(a.link) + '" target="_blank" rel="noopener noreferrer" style="color:inherit;text-decoration:none">' + esc(a.title) + '</a><br><span style="opacity:.6">' + esc(a.source) + " · " + C.ago(a.t) + '</span></span><b class="' + (s > 0 ? "up" : s < 0 ? "dn" : "") + '">' + (s > 0 ? "bullish" : s < 0 ? "bearish" : "neutral") + "</b></div>"; }).join("") : '<div class="dl-empty" style="padding:8px">' + (nc.length ? "No headline mentions " + esc(c.name) + " right now — quiet tape." : "Open the News desk once and this card fills from the same headlines.") + "</div>") +
