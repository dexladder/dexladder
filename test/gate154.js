...
(async () => {
  ...
  const COIN_SELS = ['.mkt-stats .cs .v', '.cs-range .rk-head b', '#dlRisk .ro b',
  ...
  ok('the failing statistics really are bare text on the card (transparent, no image)',
    flat['.mkt-stats .cs .v'].ownBg === 'rgba(0, 0, 0, 0)' && flat['.mkt-stats .cs .v'].ownImg === 'none' &&
    flat['.cs-range .rk-head b'].ownBg === 'rgba(0, 0, 0, 0)' && flat['#dlRisk .ro b'].ownBg === 'rgba(0, 0, 0, 0)',
    JSON.stringify([flat['.mkt-stats .cs .v'].ownBg, flat['.mkt-stats .cs .v'].ownImg]));
