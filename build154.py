...
# v162 · Every gate in this file is a bare `assert`: the input-hash lock, ~250
# patch-count checks, the dead-CSS accounting and the worker version lock. Python
# strips all of them under -O / PYTHONOPTIMIZE, and the build would still write
# dist/index.html and report success. Refuse to run in that mode.
if not __debug__:
    raise SystemExit('build154: refusing to build with assertions disabled (-O / PYTHONOPTIMIZE) '
                     '- every gate in this pipeline is an assert')
...
RELEASE = {'build': 'v163', 'name': 'the vault', 'updated': 'September 16, 2026'}

# ------------------------------------------------------------------ patch manifest
# (id, find, replace, expected count)
PATCHES = [
 # ---- 2026-09-13 · trust-centre rewrite + internal tokens renamed off another platform
 # ---- 2026-09-14 · the two social profiles that existed but were nowhere in the product
 # Instagram is already in the site's JSON-LD sameAs and Medium (@dexladder, personal profile —
 # the publication needs a paid membership) carries the five launch essays. Both were reachable
 # only from outside the app. One pair of constants, used by the footer row and by the Find
 # DexLadder row on About/Contact, so the two can never drift.
 ('social-consts',
  'GH="https://github.com/dexladder",SITE="https://dexladder.com"',
  'GH="https://github.com/dexladder",IG="https://www.instagram.com/dexladder",MED="https://medium.com/@dexladder",SITE="https://dexladder.com"', 1),
 ('social-footer',
  '>\u2325 GitHub</a><a href="mailto:\'+CONTACT+\'">\u2709 Contact</a>',
  '>\u2325 GitHub</a><a href="\'+IG+\'" target="_blank" rel="noopener">\u25ce Instagram</a><a href="\'+MED+\'" target="_blank" rel="noopener">\u24c2 Medium</a><a href="mailto:\'+CONTACT+\'">\u2709 Contact</a>', 1),
 ('social-linksrow',
  '<a class="dlt-hbtn" style="margin:0" href="mailto:\'+CONTACT+\'">\u2709 ',
  '<a class="dlt-hbtn" href="\'+IG+\'" target="_blank" rel="noopener">\u25ce instagram.com/dexladder</a><a class="dlt-hbtn" href="\'+MED+\'" target="_blank" rel="noopener">\u24c2 medium.com/@dexladder</a><a class="dlt-hbtn" style="margin:0" href="mailto:\'+CONTACT+\'">\u2709 ', 1),
 ('ui-sans-token',
  '--cmc-sans',
  '--ui-sans', 88),
 ('mkt-stats-class',
 ...
 ('mktcoin-flag',
  ...
  '__mktcoin', 2),
 ('trust-about',
  '<div class="dlt-prose"><h3>Built anonymously. On purpose.</h3><p>DexLadder has no founder photo, no LinkedIn, no company behind it — and that is not an accident. It was built by one independent developer and <b>gifted to the community</b>, in the same spirit as the pseudonymous tradition this industry was born from: the work should stand on what you can verify, not on who is asking you to trust them.</p><p>Everything this app does happens <b>inside one HTML file, on your device</b>. You can read every line with View&nbsp;Source, watch every network request it makes in your browser’s DevTools, and run it offline. There is no server, no database, no account system, no payment path, and no way to move real money — so there is nothing an identity would need to vouch for. <b>Don’t trust. Verify.</b></p><h3>Why I built this</h3><p>Most people learn crypto trading the expensive way: real savings, live leverage, no map. The exchanges that teach you are the ones that profit when you over-trade. I wanted the opposite to exist — a place with <b>the entire experience</b>: live markets, a real trading terminal, order books, P2P escrow, on-chain tracing, a full academy — where the only thing you can possibly lose is paper. Free, forever, for everyone, with no ads, no trackers and no upsell.</p><h3>Principles</h3><ul><li><b>No real money — ever.</b> Paper balances only. No deposits, no withdrawals, no token, no ICO.</li><li><b>No company, no VC.</b> Independent educational project. Nothing to monetize you with.</li><li><b>Client-side or it doesn’t ship.</b> Your data lives in your browser’s local storage and nowhere else.</li><li><b>Real data, honest labels.</b> Prices are live from public APIs; everything simulated is labeled <span class="dl-sim" style="cursor:default">Simulated</span>.</li><li><b>Open by default.</b> The code ships readable in your browser; docs and educational content are being published on GitHub.</li></ul><h3>Find DexLadder</h3>',
  '<div class="dlt-prose"><h3>Built by a trader, not a company.</h3><p>DexLadder is the work of one person: <b>Durgesh Pandey</b>. A trader, not an institution. There is no company behind it, no investors, no team — nothing in this app is anybody’s quarterly target.</p><p>It carries a name because you should know who is responsible for what you are about to learn here, and you should be able to reply to him. But the name is not the point, and it is not load-bearing. Everything this app does happens <b>inside one HTML file, on your device</b>. Read every line with View&nbsp;Source. Watch every request it makes in your browser’s DevTools. Turn the wifi off — it keeps running. There is no server, no database, no account, no password, no wallet connection and no payment path. Nothing about me has to be trusted for any of that to be true. <b>Don’t trust. Verify.</b></p><h3>Why I built this</h3><p>I came into crypto as a trader who did not understand what he was trading. Not the charts — the machinery underneath. What actually happens in the half-second after you press Buy. Who is on the other side of your fill. What the spread is quietly charging you every time you cross it. Why funding on a perpetual walks your liquidation price toward you while you sleep.</p><p>Every platform I used was built to be <i>operated</i>, not understood, and the parts that could cost me the most were explained the least. And when I went looking for the explanation, I found noise where the answer should have been: threads that shouted, people selling certainty by the month, courses charging for what one honest paragraph could have given away. The only people eager to teach me were the ones who make money when I trade too much.</p><p>So I learned it the expensive way, in the only classroom on offer — a live account, real money, and no map. This is the map. I wrote it down because nobody would, and I am not charging anyone for it.</p><h3>What this actually is</h3><p>Web3 has a doorway problem: the first step is a seed phrase you can never recover, a gas fee nobody explains, an approval you cannot read, and a transaction that will not come back. Nobody gets a first draft. DexLadder is the step before that step — it hands you the whole territory first.</p><p>A full desk: market, limit, stop, stop-limit, time-in-force, post-only, OCO and TWAP, filling against a modelled book with real spread, slippage tiers and venue-specific fees. Perpetual futures with isolated margin, tiered maintenance, and funding settled out of your margin — so your liquidation price moves toward you overnight the way it really does. Options with live greeks. AMM price impact, gas and MEV in the advanced ticket.</p><p>Then the half that is not trading at all: a contract safety scanner across seven chains, live DEX pool radar, TVL by chain, a universal tracer that identifies any address or transaction hash across fifteen chains and follows it, a mempool radar, and a read-only wallet mirror — <b>there is no wallet connection in this app, and there never will be</b>. That is not a missing feature; it is why this is a safe first door.</p><p>Underneath, the cryptography is not decorative. Every action is sealed into a SHA-256 hash chain. A Merkle proof explorer and an SPV demonstration show how a light client verifies what it never downloaded. Your certificate and reports are signed with a real secp256k1 key generated on your device and never transmitted — and you can export a standalone verifier containing none of this app’s code to re-audit your ledger offline, with DexLadder deleted.</p><p><b>Everything in here is real except the money — and that is the point.</b> The chains are real. The contracts you scan are real. The liquidation maths is real. The one thing that is not real is the one thing that could have hurt you.</p><h3>What that promise cost</h3><ul><li><b>A five-source price ladder.</b> One feed going dark should degrade this app, never let it quietly invent a number.</li><li><b>Every API keyless.</b> No key means no account, no quota, no vendor able to switch you off — and no reason for a paid tier to ever appear here.</li><li><b>A verifier that contains none of my code</b>, so you can audit your own ledger with this app deleted. Making it unnecessary took real work. That was the point.</li><li><b>A tamper test that runs on you, in front of you</b> — it edits one field in a copy of your ledger and shows the verification shatter at that exact block.</li><li><b>Honest labels where they cost.</b> Per-trade Sharpe is labelled trade quality, not fund maths, and the backtester states its own fill rules rather than flattering you.</li><li><b>An Oracle that admits ignorance</b> instead of inventing an answer. That is harder to build than something which always has an opinion.</li></ul><p>None of that makes me trustworthy. It makes me <b>unnecessary</b> — which is the only honest thing a free app in this industry can offer you.</p><h3>Principles</h3><ul><li><b>Nothing at stake.</b> No deposits, no withdrawals, no token, no ICO, no wallet connection.</li><li><b>Independent.</b> One person. No company, no VC, no sponsors, no affiliate links.</li><li><b>Client-side or it doesn’t ship.</b> Your data lives in your browser’s local storage and nowhere else.</li><li><b>Keyless.</b> Every data source is a public, keyless API.</li><li><b>Real data, honest labels.</b> Prices, chains, contracts and pools are live; everything modelled is labeled <span class="dl-sim" style="cursor:default">Simulated</span>.</li><li><b>Free permanently — not free for now.</b> No paid tier behind this, and no plan to add one.</li></ul><h3>Find DexLadder</h3>', 1),
 ('trust-paper7',
  '<h3>7 · Why free and why anonymous</h3><p>DexLadder is a gift to the community. No company, no VC, no token. The builder stays pseudonymous so the project has to earn trust the only way that scales: by being <b>inspectable</b>. Open the file. Watch the network tab. Pull the plug and watch it keep working.</p>',
  '<h3>7 · Why it is free, and who built it</h3><p>DexLadder is free because every alternative corrupts it. A paid tier bends the lessons toward what sells the tier; ads make the objective time-on-screen rather than competence; payment of any kind needs a server, an account system and a database of who you are. Charging nothing is the cheapest way to keep the incentives clean — and it is structurally possible, because every data source here is a public keyless API and there is no server, so nothing costs money to run per user.</p><p>It was written by one person, <b>Durgesh Pandey</b>, a trader who learned this machinery at his own expense and concluded that the way in should not have a price on it. The name is here because you deserve to know whose reasoning you are reading — not as evidence. Every claim in this paper is checkable on your own device in five minutes, and §8 tells you how. A name is accountability; §8 is the proof.</p>', 1),
 ('trust-contact',
  'DexLadder is built and maintained by one independent, pseudonymous developer. Real inbox, real replies',
  'DexLadder is built and maintained by one person, <b>Durgesh Pandey</b>. Real inbox, real replies', 1),
 ('trust-footer-id',
  '<div class="dlt-id">Built anonymously and <b>gifted to the community</b>',
  '<div class="dlt-id">Built by one trader and <b>gifted to the community</b>', 1),
 ('cl-v143',
  '"Day-mode click tone matched to CMC · ivory lesson stages · 1352px container"',
  '"Day-mode click tone tuned to the light system · ivory lesson stages · 1352px container"', 1),
 ('cl-v141',
  '"Full CMC-style light system · instant service-worker updates · glass boot exit"',
  '"Full daylight colour system · instant service-worker updates · glass boot exit"', 1),
 ('cl-v127',
  '"CMC-style market structure: subnav, 500-coin universe, compare, watchlist, categories"',
  '"Market structure rebuilt: subnav, 500-coin universe, compare, watchlist, categories"', 1),
...
NEWS_V163 = "THE VAULT. The desk stops forgetting. Until now every fill, journal entry, alert and equity point lived in one browser key that had to be pruned to fit — the last 50 fills, 60 journal entries, 40 alerts and 120 points of the equity curve, and less than that the first time the browser refused a write. Trade for three months and you kept the last fifty trades. The working desk still runs on that key, unchanged and just as fast; beside it there is now an archive in IndexedDB that keeps all of it, in order, for as long as you keep this site's data. Nothing you already had was touched: the same code saves your desk, the same renderers draw it, and the archive is written after the save rather than inside it — so if your browser refuses IndexedDB entirely, the app behaves exactly as it did before. YOUR BACKUP GREW UP WITH IT. The signed bundle you export from the Sovereign panel used to carry one key and the pruned history inside it. It now carries every DexLadder setting on this browser and the whole archive, counted on the file so you can see what is in it before you open it, and checksummed with SHA-256 so a truncated or edited file is refused before its signature is even checked. It is the same secp256k1 key and the same signature scheme as before, so anything that verified the old bundle verifies this one, and older bundles still import. Your signing key is left out unless you tick the box that asks for it, because an export you send to someone should not carry it; tick it and the other device becomes this desk. An import writes the desk it replaced into the archive first, so it is reversible — Put it back is a button. AND THE PANEL STATES WHAT IS ACTUALLY THERE: rows archived against rows the app is holding, bytes on this origin, and whether this browser will still accept a write — measured here, not assumed from a number someone guessed. You do not have to take any of that on trust. Open your browser console on this page and run await DLVAULT.ready; DLVAULT.writable() — it answers true when the archive is on. DLVAULT.stats() prints what is in it. DLVAULT.panel() opens the same figures with export and import beside them. Every one of those numbers came off your own machine, because there is no server to ask."
TRUST_OLD = '<h3>What is stored on your device</h3><p>localStorage keys hold your paper portfolio and trades, journal, alerts, academy progress and XP, certificate name (if you set one), display preferences (theme, mode, contrast), cached prices, and the locally generated signing key. <b>To erase everything:</b> clear this site’s data in your browser (or uninstall the app). Nothing survives, because nothing was ever anywhere else.</p>'
TRUST_NEW = '<h3>What is stored on your device</h3><p>Two stores, both on this device and nowhere else. <b>localStorage</b> holds the working desk: your paper portfolio and open trades, journal, alerts, academy progress and XP, certificate name (if you set one), display preferences (theme, mode, contrast), cached prices, and the locally generated signing key. <b>IndexedDB</b>, in a database named <b>dexladder-vault</b>, holds the archive: the full history of fills, journal entries, alerts and the equity curve — kept past the limits the working desk has to prune to in order to fit a browser key. <b>To erase everything:</b> clear this site’s data in your browser (or uninstall the app), which clears both. Nothing survives, because nothing was ever anywhere else.</p><h3>Checking that instead of believing it</h3><p>Open your browser console on this page and run <b>await DLVAULT.ready; DLVAULT.writable()</b> — it answers true when the archive is on and this browser is accepting writes to it. <b>DLVAULT.stats()</b> prints the rows archived, the bytes this origin holds and the quota your browser reports. <b>DLVAULT.panel()</b> opens the same figures with export and import beside them. None of those calls reach a network; there is no server to reach.</p><h3>Taking it with you</h3><p>The Sovereign panel exports one file: every DexLadder setting on this browser, the whole archive, a SHA-256 checksum over the lot, and a signature from your own key. The checksum is verified before the signature, so a truncated or edited file is refused even on a browser where signature checking is unavailable. Your signing key is left out unless you ask for it — tick that box only when you are cloning this desk onto another device you own. Importing writes the desk it replaced into the archive first, so it can be undone.</p>'
PROFILE_OLD = 'Your profile (a display name and a locally generated key) is stored only in localStorage.'
PROFILE_NEW = 'Your profile (a display name and a locally generated key) is stored only on this device, in localStorage.'
BAL_OLD = '<li><b>Balances:</b> stored only in localStorage. Deleting site data resets everything — nothing exists anywhere else.</li>'
BAL_NEW = '<li><b>Balances:</b> stored only on this device — the working balance in localStorage, the full trade history in the dexladder-vault archive. Deleting this site’s data resets both; nothing exists anywhere else.</li>'

# ---- 2026-09-16 · v163 · THE DESCRIPTION GAP
# The vault shipped in the payload and nothing a reader could see said so. An
# outside assessor reading dexladder.com concluded, correctly from what was in
# front of them, that state was "still localStorage + a signed backup". Two
# separate failures, both fixed here:
#   1. the What's-New banner carried v162's audit text under a v163 title,
#      because the body was a literal typed into the patch above while only the
#      TITLE derives from RELEASE. The v163 entry now describes v163, and the
#      audit text becomes v162's own entry rather than being overwritten.
#   2. the Trust Centre described one store. There are two, and the second is
#      the whole point of this release. It now says so, and hands the reader the
#      three console calls that check it rather than asking to be believed.
PATCHES += [
 ('v163-news-vault',
  'var NEWS=[["%(build)s \u00b7 %(name)s","THE LIBRARY.' % RELEASE,
  'var NEWS=[["%(build)s \u00b7 %(name)s","' % RELEASE + NEWS_V163 + '"],["v162 \u00b7 the colonnade","THE LIBRARY.',
  1),
 ('v163-trust-stored', TRUST_OLD, TRUST_NEW, 1),
 ('v163-trust-profile', PROFILE_OLD, PROFILE_NEW, 1),
 ('v163-trust-balances', BAL_OLD, BAL_NEW, 1),
]
...
#
#   .mkt-stats .cs .v      market cap · volume 24h · volume/mcap · FDV · circulating ·
...
_rootpal = re.search(r':root\{--ui-sans:[^}]*\}', out)
...
# DexChart Pro: the native Terminal chart becomes a full charting engine (layer half in
# layers/46-dexchart.js, chrome in layers/v154.css). Runs last so it patches the payload the
# v156 history hook already edited.
import buildlib.dxc as _dxc
out = _dxc.apply(out)

# v162 audit pass. Runs last, against the near-final payload, so every anchor is
# matched against bytes that actually ship. Closes the findings of the full
# line-by-line audit that are in the base payload rather than in a layer:
# the two new Function() sinks, boot()'s all-or-nothing comma chain, the missing
# global error net, the cross-tab ledger clobber, the silent save failure, the
# hidden-tab engine and its 240-tick refocus freeze, an invalid reduced-motion
# time value, unnamed dialogs and close buttons, duplicate gradient ids, Infinity
# rendering as text, and the lead copy that sold a simulator.
import buildlib.v162 as _v162
out = _v162.apply(out)
